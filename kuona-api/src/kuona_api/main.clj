...
(def cli-options
  [["-c" "--config FILE" "API Key used to communicate with the API service (deprecated/ignored)" :default "kuona-properties.json"]
   ...
   ["-m" "--mode MODE" "Run mode standalone/passive. In standalone mode local scheduler runs to collect data. In passive mode external collection is assumed" :default "standalone"]
